...
class MetricCalculation {
  static PercentageVideoCompleted(sessions) {
    ...
    if (sessions == null || sessions === undefined || sessions.length === 0) {
      return {
        procastinationCounter: 0,
        nonProcastinationCounter: 0,
        totalVideos: 0,
      };
    }
    ...
    sessions.map((session) => {
      if (
        YoutubeVideoCategorization.isProcastinationVideo(
          session.videoMetadata.videoCategory
        )
      ) {
  ...
  static formatSessionDate(sessions) {
    let categoryByDate, groupedResult;
    ...
    categoryByDate = sessions.map((session) => {
      ...
      return {
        date: stringDate,
        videoCategory: session.videoMetadata.videoCategory,
      };
    });
    ...
    return groupedResult.map((resultGroup) => {
      return {
        ...
        categoryData: this.AggregateCategoryCounter(resultGroup),
      };
    });
  }
