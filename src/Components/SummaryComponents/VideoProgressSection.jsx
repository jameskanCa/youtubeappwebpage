...
export default class VideoProgressSection extends React.Component {
  prepareGraphData() {
    let percentageCompleted = MetricCalculation.PercentageVideoCompleted(
      this.props.sessions
    );
    return [
      {
        id: "Completed Videos by %",
        label: "% Completed Videos",
        value: (100 * percentageCompleted).toFixed(0),
        color: "hsl(69, 70%, 50%)",
      },
      {
        id: "Incomplete Videos by %",
  ...
  prepareProcastinationData() {
    let procastinationMetric = MetricCalculation.ProcastinationCounter(
      this.props.sessions
    );
    const procastinationPercentage =
      procastinationMetric.procastinationCounter /
      procastinationMetric.totalVideos;
    const nonProcastinationPercentage =
      procastinationMetric.nonProcastinationCounter /
      procastinationMetric.totalVideos;
    return [
      {
        id: "Procastination Videos by %",
        label: "% Procastiation Videos",
        value: (100 * procastinationPercentage).toFixed(0),
        color: "hsl(69, 70%, 50%)",
      },
      {
        id: "NonProcastination Videos by %",
